...
# what lets a SECOND, DIFFERENT chest opened in the exact same screen
# spot (common when rapidly farming -- one chest closes and the next
# opens before the ABSENCE_CONFIRM_SECONDS + post_close_cooldown_s gap
# would otherwise elapse) get caught and logged immediately, instead of
# only ever being caught after the full close/cooldown cycle. Throttled
# independently of the (often much faster) poll interval since this does
# a real OCR pass, not just a cheap pixel-coverage check.
...
class DetectorSettings:
    ...
    # for a new popup at the same spot. This used to default to 2.0s (on
    # top of the old 1.2s ABSENCE_CONFIRM_SECONDS) and wasn't
    # user-adjustable, which meant detection couldn't pick up a genuinely
    # new chest for up to ~3.2s after the last one closed -- too slow for
    # players who loot quickly. Lowered and exposed in Settings; the
    # CONTENT_RECHECK mechanism below now catches most same-spot rapid
    # reopens well before this cooldown would even matter, so this is
    # mainly a safety net for the fade-animation case it was originally
    # added for.
...
class LootDetector:
    ...
    def __init__(
        ...
        # CURRENT loot window instance, so a periodic re-read while the
        # window is still open can tell "player is taking items one at a
        # time / hit Take Small Items" (count/gold only ever shrinks from
        # here on -- never re-logged) apart from "a new, different chest
        # opened in this same screen spot" (count or gold goes ABOVE
        # anything seen so far -- only possible if the old popup was
        # actually replaced by a new one, since taking items can never add
        # to what's on screen). Deliberately count-based rather than
        # tracking individual item names: two different chests can easily
        # drop the same-named item (including two different Famed/
        # Legendary drops in a row), and a name-based "already seen this
        # exact name" check would wrongly swallow the second one instead
        # of logging it. Reset to 0 whenever the window is confirmed
        # closed. See CONTENT_RECHECK_INTERVAL_S and
        # _check_for_new_chest_content.
        self._seen_item_count = 0
        self._seen_gold = 0
        self._last_content_check_at = 0.0
    ...
    def _scan_once(self, sct, monitor):
        ...
        if self._window_present_last and self._last_known_box is not None:
            ...
            if self._region_still_present(mask, self._last_known_box):
                ...
                if now - self._last_content_check_at >= CONTENT_RECHECK_INTERVAL_S:
                    self._last_content_check_at = now
                    self._check_for_new_chest_content(frame, self._last_known_box)
                return
        ...
        if result is not None:
            self._seen_item_count = len(result.items)
            self._seen_gold = result.gold
    ...
    def _check_for_new_chest_content(self, frame: np.ndarray, box) -> None:
        """
        ...
        If instead the item count or gold reads HIGHER than anything seen
        so far, that can only mean a fresh, different chest opened in this
        same screen spot before the old one was confirmed closed (taking
        items never adds more to the screen) -- most likely because the
        whole open-close-reopen happened faster than
        ABSENCE_CONFIRM_SECONDS could register the popup as ever having
        gone away. In that case the entire current read is logged as a new
        ChestResult. This is deliberately NOT restricted to just the
        "new" items by name -- two different chests can easily drop an
        item with the same name (including two different Famed/Legendary
        drops in a row), and by the time a new chest's popup is showing,
        the previous one's popup (and its now-claimed-or-abandoned items)
        is already gone, so the full current read IS the new chest's
        contents.
        """
        ...
        current_count = len(reread.items)

        if current_count > self._seen_item_count or reread.gold > self._seen_gold:
            print(f"[TLOPO detect] new chest content detected at same spot: "
                  f"items={[i.name for i in reread.items]} gold={reread.gold} "
                  f"(previous seen_count={self._seen_item_count} seen_gold={self._seen_gold})", flush=True)
            self.on_chest_detected(reread)
            self._seen_item_count = current_count
            self._seen_gold = reread.gold
        # else: current read is a subset of (or equal to) what's already been
        # logged -- items being taken, or a one-frame OCR undercount. Baseline
        # is intentionally NEVER lowered here: if it tracked the current
        # (possibly transient) low count instead of the running max, a later
        # frame simply reading the same still-open chest correctly again
        # would look like "growth" and get wrongly logged a second time.
