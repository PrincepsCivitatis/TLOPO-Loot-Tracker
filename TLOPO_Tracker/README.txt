...
- If you're looting quickly and a chest right after another one closes
  doesn't get picked up: open Settings and lower the "Chest close
  cooldown" number. This controls how long the tracker waits after a
  chest closes before it'll count a new one appearing in the same spot
  - the default is short enough for most play, but if you're looting
  faster than that, lowering it (down to 0ms if needed) closes the gap.
